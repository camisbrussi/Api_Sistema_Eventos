...
class RegistrationController {
  async store(req, res) {
    try {
      ...
      const exists_registration = await _Registration2.default.findOne( {where: {customer_id, event_id}});
      ...
      const newRegistration = await _Registration2.default.create({
        customer_id,  
        event_id, 
        auth_key, 
        status
      ...
      return res.json({sucess:'Registrado com sucesso'});
